 ...
 * definitions
 */
define( 'THEME_ASSIGN_FMT_MENU_ID' , 'menu_level%d_%s_%s' ) ;
...
/** Avoid a  flash of unstyled content (FOUC) by using server-side script PHP.
 * Instead of saving the user’s theme preference in localStorage,
 * we will create a cookie from JavaScript and save it there.
 */

/**
 * Check for theme cookie and load the appropriate theme by applying the class to the <body> tag
...
if (!empty($_COOKIE['theme'])) {
	if ($_COOKIE['theme'] == 'dark-theme') {
		...
		//$themeClass = 'dark';
	} else if ($_COOKIE['theme'] == 'light-theme') {
...
$checkCookie = '';
// if the cookie "theme" is set
if ( isset( $_COOKIE['theme'] ) ) {
	$checkCookie = "<p>PHP cookie value for <i>theme</i>:<b>" . $_COOKIE['theme'] . "</b></p>";
} else {
	$checkCookie = "<p>PHP <b>No cookie</b> value for <i>theme</i>!</p>";
}

/**
 * or using the separate stylesheets method
...
if ( ! is_array( @$this->_tpl_vars['xoops_breadcrumbs'] ) ) {
	...
	if ( ! is_object( @$xoopsModule ) ) {
		...
		if ( isset( $modcat_assigns[ $dirname ] ) && strlen( $tplvar = theme_assign_get_tpl_vars( $this, $modcat_assigns[ $dirname ][0] ) ) ) {
			...
			if ( $tplvars_info[2] ) {
				theme_assign_clear_tpl_vars( $this, $tplvars_info[0] );
			}
		} else if ( isset( $modcat_trees[ $dirname ] ) ) {
			// category tree (the last method)
			$tree_info = $modcat_trees[ $dirname ];
			if ( @$_GET[ $tree_info[4] ] > 0 ) {
				$id_val = (int) $_GET[ $tree_info[4] ];
			} else if ( ! empty( $tree_info[5] ) ) {
				$id_val = theme_assign_get_tpl_vars( $this, $tree_info[5] );
			}
			if ( ! empty( $id_val ) ) {
				$breadcrumbs = array_merge(
					$breadcrumbs,
					theme_assign_get_breadcrumbs_by_tree( $tree_info[0],
						$tree_info[1], $tree_info[2],
						$tree_info[3], $id_val,
						XOOPS_URL . '/modules/' . $dirname . '/' . $tree_info[6] ) );
			}
		}
		if ( ! in_array( @$this->_tpl_vars['xoops_pagetitle'], array(
			$modname,
 ...
 * @return array|mixed
 */
function theme_assign_get_breadcrumbs_by_tree( $table, $id_col, $pid_col, $name_col, $id_val, $url_fmt, $paths = array() ) {

	$db =& Database::getInstance();

	$sql    = "SELECT `$pid_col`,`$name_col` FROM " . $db->prefix( $table ) . " WHERE `$id_col`=" . (int) $id_val;
	$result = $db->query( $sql );
	...
	return theme_assign_get_breadcrumbs_by_tree( $table, $id_col, $pid_col, $name_col, $pid, $url_fmt, $paths );
}
 ...
 * @return mixed
 */
function theme_assign_get_tpl_vars( &$smarty, $dot_expression ) {
	...
	foreach ( $groups as $group ) {
		...
		foreach ( $modules as $module ) {
			if ( in_array( $module->getVar( 'mid' ), $read_allowed ) ) {
				...
				theme_assign_process_sub_recursively( $sub, $module->getVar( 'dirname' ) );
...
foreach ( $groups4assign as $group_tmp ) {
	if ( is_array( @$menus[ $group_tmp['id'] ] ) ) {
		$united_menus += $menus[ $group_tmp['id'] ];
	}
}

$this->assign( 'theme_menus', $united_menus );
 ...
 * @param int $level
 */
function theme_assign_process_sub_recursively( &$items, $dirname = '', $level = 1 ) {
	if ( is_array( $items ) ) {
		foreach ( array_keys( $items ) as $i ) {
			$items[ $i ]['dirname'] = $dirname;
			$items[ $i ]['id']      = sprintf( THEME_ASSIGN_FMT_MENU_ID, $level, $dirname, substr( md5( $items[ $i ]['url'] ), - 4 ) );
			$items[ $i ]['class']   = sprintf( XUGJ_ASSIGN_FMT_MENU_CLASS, $level, $dirname );
			if ( ! empty( $items[ $i ]['sub'] ) ) {
				theme_assign_process_sub_recursively( $items[ $i ]['sub'], $dirname, $level + 1 );
			}
		}
	}
}
...
// theme_menu_uls
$this->assign( 'theme_menu_uls', theme_assign_display_menu_ul_recursively( $united_menus ) );
 ...
 * @return string
 */
function theme_assign_display_menu_ul_recursively( $level_menus, $dirname = '' ) {
	$ret = "<ul>\n";
	foreach ( $level_menus as $menu ) {
		if ( ! empty( $menu['dirname'] ) ) {
			$dirname = $menu['dirname'];
		}
		$ret .= "<li>\n<a href=\"" . XOOPS_URL . '/modules/' . $dirname . '/' . $menu['url'] . "\">" . $menu['name'] . "</a>\n";
		if ( ! empty( $menu['sub'] ) && is_array( $menu['sub'] ) ) {
			$ret .= theme_assign_display_menu_ul_recursively( $menu['sub'], $dirname );
		}
		$ret .= "</li>\n";
	}
